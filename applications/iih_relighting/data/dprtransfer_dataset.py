...
class DPRTransferDataset(BaseDataset):
        ...
        if opt.isTrain==True:
            ...
            with open(self.trainfile,'r') as f:
                for line in f.readlines():
                    ...
                    if file_name_arr[1] != file_name_arr[2]:
                        ...
                        if last_dir != file_name_arr[0]:
                            ...
                            self.dir_paths.append(file_name_arr[0])
        elif opt.isTrain==False:
            #self.real_ext='.jpg'
            print('loading test file')
            # self.trainfile = opt.dataset_root+'test.lst'
            self.trainfile = opt.dataset_root+'randomdprtgt_test.txt'
            with open(self.trainfile,'r') as f:
                for line in f.readlines():
                    file_names = line.rstrip()
                    file_name_arr = file_names.split(" ")
                    self.image_paths.append(os.path.join(opt.dataset_root,'DPR_dataset',file_name_arr[0]))
                    self.real_image_paths.append(os.path.join(opt.dataset_root,'DPR_dataset',file_name_arr[1]))
    ...
    def __getitem__(self, index):
        ...
        real_path_tmp = real_path.split("/")
        path = path[:-4]+"-"+real_path_tmp[-1]


        # target_num = random.randint(0, len(self.dir_paths)-1)
        # target_dir = self.dir_paths[target_num]
        # real_path_arr = real_path.split("/")
        # target_path = real_path.replace(real_path_arr[-2], target_dir)
        # target = Image.open(target_path).convert('RGB')
        # target = self.transforms(target).to(torch.float32)
        # target_light_path = target_path.replace('_0', "_light_0").replace(".png",".txt")
        # target_light = np.loadtxt(target_light_path)
        # target_light = target_light[0:9]
        # target_light = np.squeeze(target_light).astype(np.float32)
        # target_light = torch.from_numpy(target_light)

        return {'fake_light': fake_light, 'real_light':real_light, 'target_light':real_light, 'fake': fake, 'target':real, 'real':real, 'img_path':path}
