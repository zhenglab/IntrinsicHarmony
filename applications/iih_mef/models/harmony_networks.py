...
class BaseGDGenerator(nn.Module):
    def __init__(self, opt=None):
        ...
        i_enc_n_res = 4
        i_dec_n_res = 0
        self.illumination_ifm = opt.illumination_ifm

        self.reflectance_enc = ContentEncoder(opt.n_downsample, r_enc_n_res, opt.input_nc*2, self.reflectance_dim, opt.ngf, 'in', opt.activ, pad_type=opt.pad_type)
        ...
        self.ifm = InharmonyfreeCAModule(opt.n_downsample+1, opt.ifm_n_res, opt.input_nc*2, self.reflectance_enc.output_dim, opt.ngf//2, opt.inharmonyfree_norm, opt.activ, pad_type=opt.pad_type,lamda=opt.lamda)

        self.reflectanceRec = HarmonyRecBlocks(opt.inharmonyfree_embed_layers, dim=self.reflectance_enc.output_dim)
        self.act = nn.Tanh()

    def forward(self, inputs_u,inputs_o):
        inputs = torch.cat([inputs_u, inputs_o], dim=1)
        match_score, ifm_mean = self.ifm(inputs)

        r_content = self.reflectance_enc(inputs)
        i_content_u = self.illumination_enc(inputs_u)
        i_content_o = self.illumination_enc(inputs_o)

        r_content = self.reflectanceRec(r_content, attScore=match_score)

        reflectance = self.reflectance_dec(r_content)

        illumination_u = self.illumination_dec(i_content_u)
        illumination_o = self.illumination_dec(i_content_o)
        rec_u = self.act(reflectance*illumination_u)/2+0.5
        rec_o = self.act(reflectance*illumination_o)/2+0.5
        return rec_u, rec_o, reflectance, ifm_mean
