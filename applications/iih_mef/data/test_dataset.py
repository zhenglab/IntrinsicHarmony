...
class TestDataset(BaseDataset):
        ...
        if opt.isTrain==True:
            ...
            with open(self.trainfile,'r') as f:
                    for line in f.readlines():
                        self.image_paths.append(os.path.join(opt.dataset_root,'images',line.rstrip()))
                        # self.image_paths.append(os.path.join(opt.dataset_root,'Expert_B',line.rstrip()))
                        # self.image_paths.append(os.path.join(opt.dataset_root,'Expert_D',line.rstrip()))
                        # self.image_paths.append(os.path.join(opt.dataset_root,'Expert_E',line.rstrip()))
    ...
    def __getitem__(self, index):
        ...
        # target_path = self.image_paths[index].replace(name_parts[-2],'high')
        # if self.isTrain:
        #     target_path = self.image_paths[index].replace(name_parts[-2],'high')
        # else:
