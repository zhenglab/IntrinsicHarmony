...
def define_G(netG='retinex',init_type='normal', init_gain=0.02, opt=None):
    ...
    """
    if netG == 'retinex':
        net = retinexGenerator(opt)
    elif netG == 'retinex_ifpm':
        net = retinexifpmGenerator(opt)
    elif netG == 'retinex_ltm':
        net = retinexltmGenerator(opt)
    elif netG == 'retinex_ltm_ifm':
        net = retinexltifpmGenerator(opt)
    else:
